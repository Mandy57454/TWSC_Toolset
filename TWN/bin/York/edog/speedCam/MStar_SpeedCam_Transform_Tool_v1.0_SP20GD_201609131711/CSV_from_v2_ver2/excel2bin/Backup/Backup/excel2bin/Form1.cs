...
{  
    public partial class Form1 : Form
    {
        ...
        bool doubleorfloat = false;   //if true: double version; if false: float version; 
        
        // for dynamic slicing layers
        bool dynamicslicing = true;   // if false: average slicing; if true: dynamic
        bool IsCHNPOI = false;     // if true: for CHN POI data; if true: for other POI data        
        ...
        public void LoadExcelFile(string excelfile)
        {
            ...
            if (File.Exists(excelfile))
            {
                try
                {
                    ...
                    {
                        /*
                        string ConnectionString = string.Format(@"Provider=Microsoft.Jet.OLEDB.4.0;Data Source={0}; Extended Properties=""text;HDR=Yes;FMT=Delimited"",", excelfile); 
                        string CommandText = string.Format(@"SELECT * FROM " + excelfile, ConnectionString);  //FileUpload1.FileName就是檔案的名子就好，範例是結合上傳的檔案
                        ...
                        CSVConnection.Open();
                        CSVAdapter.Fill(dTableOut1);
                        CSVConnection.Close();
                       
                        var lines = File.ReadAllLines(excelfile).Select(a => a.Split(',')).ToList();
                        csvfilelines = int.Parse(lines[0][0].ToString());
                        mioversion = uint.Parse(lines[1][0].ToString());
                        for (int j = 1; j <= csvfilelines; j++)
                        {
                            for (i = 0; i < lines[j].Length; i++)                                
                                dTableOut1.Rows.Add(lines[1+j][i]);
                        }
                        */
                        ...
                        for (i = 1; i <= csvfilelines; i++)
                        {
                            ...
                            if (tt.Length <= 7)
                            {
                                ...
                                {                                 
                                    if ((double.Parse(tt[k])) < 10.0)
                                        tt[k] = "00" + tt[k];
                                    else if ((double.Parse(tt[k])) < 100.0)
                                        tt[k] = '0' + tt[k];
                                }
                    ...
                    if (latsmallzero)
                        for (i = 0; i < DGV_excel.RowCount; i++)
                            DGV_excel.Rows[i].Cells[1].Value = (double.Parse(DGV_excel.Rows[i].Cells[1].Value.ToString()) + 90).ToString();

                    if (lonsmallzero)
                        for (i = 0; i < DGV_excel.RowCount; i++)
                            DGV_excel.Rows[i].Cells[2].Value = (double.Parse(DGV_excel.Rows[i].Cells[2].Value.ToString()) + 180).ToString();
                    */
        ...
        public void speedcamconvert()
        {
            ...
            bool IsPOIsuminsmallblocklessthanthreshold = false;  //false: the original division; true: new division based on 20140305
            ...
            int POIsuminsmallblockthreshold = 3000;       //3000
            ...
            if (IsCHNPOI) //DigiLife CHN use this
            {
                POIsuminsmallblockthreshold = 5500;                
                ...
                //DivisionEvaluation.DGV_verifydivision.RowCount = 20 * 30;              // 20140421
                //DivisionEvaluation.DGV_verifydivision.ColumnCount = 20 * 30;      // 20140421
                ...
                if (lB_nestedslicingrules.Items.Count > 0)                
                {
                                ...
                                if ((!failedcombination) && (divisionblockcount[r][0].ToString().Length > 0) && (multilevelpoints[r, 0, 0] == null))
                                {
                                    ...
                                    {                                    
                                        for (j = 1; j < (tmplonblockcount * tmplonsmallblockcount - 1); j++)
                                        {
                                            ...
                                            if (tmpnineblockPOI >= POIsuminsmallblockthreshold)
                                            {
                                                string ttttt = "Subarea : " + r.ToString() + "; Div: " + t.ToString() + ", " + s.ToString() + " ; index: " + i.ToString() + ", " + j.ToString() + "; POI: " +tmpnineblockPOI.ToString();
                                                POIlargerthan3000.Add(ttttt);
                                            }
                                        }
                    ...
                    //for (t = 11; t <= 19; t++)                    // for CHN, 20140421
                    {
                        tB_BlockCount.Text = t.ToString();
                        tB_LonBlockCount.Text = t.ToString();
                        tmplatblockcount = t;
                        ...
                        //for (s = 19; s <= 29; s++)    // for CHN, 20140421
                        {
                            tmplatsmallblockcount = s;
                            tmplonsmallblockcount = s;
                            tB_SmallBlockCount.Text = tmplatsmallblockcount.ToString();
                            ...
                            */
                            ...
                            {                                
                                ...
                                for (i = 1; i < (tmplatblockcount * tmplatsmallblockcount - 1); i ++)
                                {
                                    for (j = 1; j < (tmplonblockcount * tmplonsmallblockcount - 1); j++)
                                    {
                                        ...
                                        //if (tmpnineblockPOI >= POIsuminsmallblockthreshold)
                                        tmppoicount[i, j] = tmpnineblockPOI;
                                    }
                                }
                            }
                            ...
                            long ttt = 0;
                            int tmpi = 0, tmpj = 0;
                            for (i = 0; i < tmppoicount.GetLength(0); i++)
                                for (j = 0; j < tmppoicount.GetLength(1); j++)
                                    if (tmppoicount[i, j] > ttt)
                                    {
                                        ttt = tmppoicount[i, j];
                                        tmpi = i;
                                        tmpj = j;
            ...
            else if ( ( (IsPOIsuminsmallblocklessthanthreshold) && (POIsuminsmallblockthreshold > 0) ) && (IsCHNPOI == false) )
            {                               
                failedcombination = false;
                DivisionEvaluation = new Fm_evaluatedivision();
                DivisionEvaluation.Hide();
                ...
                for (r = 10; r < testroundnum; r++)
                {
                    tB_BlockCount.Text = r.ToString();
                    for (s = 10; s < testroundnum; s++)
                    {
                        tmplatblockcount = r;
                        tmplonblockcount = s;
                        tB_LonBlockCount.Text = s.ToString();
                        
                        //tmplonblockwidth = Math.Round((double)(lonend - lonstart) / tmplonblockcount, 4);
                        tmplonblockwidth = (double)(lonend - lonstart) / tmplonblockcount;
                        //tmplatblockwidth = Math.Round((double)(latend - latstart) / tmplatblockcount, 4);
                        tmplatblockwidth = (double)(latend - latstart) / tmplatblockcount;
                        tmplatsmallblockcount = 3;
                        tmplonsmallblockcount = 3;
                        tB_SmallBlockCount.Text = tmplatsmallblockcount.ToString();
                        tB_LonSmallBlockCount.Text = tmplonsmallblockcount.ToString();                        
                        //tB_BlockCount.Refresh();
                        //tB_LonBlockCount.Refresh();
                        ...
                        //tmplonsmallblockwidth = Math.Round((double)tmplonblockwidth / tmplonsmallblockcount, 4);
                        tmplonsmallblockwidth = (double)tmplonblockwidth / tmplonsmallblockcount;
                        //tmplatsmallblockwidth = Math.Round((double)tmplatblockwidth / tmplatsmallblockcount, 4);
                        tmplatsmallblockwidth = (double)tmplatblockwidth / tmplatsmallblockcount;
                        ...
                        for (i = 1; i < (DivisionEvaluation.DGV_verifydivision.RowCount - 1); i += 3)
                        {
                            for (j = 1; j < (DivisionEvaluation.DGV_verifydivision.ColumnCount-1); j++)
                            {
                                if ((long.Parse(DivisionEvaluation.DGV_verifydivision.Rows[i - 1].Cells[j - 1].Value.ToString()) +
                                     long.Parse(DivisionEvaluation.DGV_verifydivision.Rows[i].Cells[j - 1].Value.ToString()) +
                                     long.Parse(DivisionEvaluation.DGV_verifydivision.Rows[i + 1].Cells[j - 1].Value.ToString()) +
                                    ...
                                    long.Parse(DivisionEvaluation.DGV_verifydivision.Rows[i - 1].Cells[j + 1].Value.ToString()) +
                                     long.Parse(DivisionEvaluation.DGV_verifydivision.Rows[i].Cells[j + 1].Value.ToString()) +
                                     long.Parse(DivisionEvaluation.DGV_verifydivision.Rows[i + 1].Cells[j + 1].Value.ToString())) >= POIsuminsmallblockthreshold)
                                {
                                    failedcombination = true;
                                    break;
                                }
        ...
        public void SaveExcelFile(string excelfile)
        {
            ...
            if (IsCHNPOI)
            {
                ...
                for (filecounter = 0; filecounter < multilevelpoints.GetLength(0); filecounter++)
                {
                    ...
                    // 1. Global header            
                    outputwriter.Write((ushort)lonstart); // Start position of Longitude
                    if (Isdebug) debuglist.Add(lonstart);
                    outputwriter.Write((ushort)latstart);  // Start position of Latitude
                    if (Isdebug) debuglist.Add(latstart);
                    outputwriter.Write((ushort)lonend);  // End position of Longitude
                    if (Isdebug) debuglist.Add(lonend);
                    outputwriter.Write((ushort)latend);   // End position of Latitude
                    ...
                    for (i = 0; i < blockcount * lonblockcount; i++)
                    {
                        ...
                        outputwriter.Write((ushort)(latstart + tmplatindex * layerwidth));                    // Start Latitude
                        if (Isdebug) debuglist.Add(latstart + tmplatindex * layerwidth);
                        outputwriter.Write((ushort)(lonstart + tmplonindex * lonlayerwidth));             // Start Lontitude
                        if (Isdebug) debuglist.Add(lonstart + tmplonindex * lonlayerwidth);

                        if ((latstart + tmplatindex * layerwidth + layerwidth) < latend)
                        {
                            outputwriter.Write((ushort)(latstart + tmplatindex * layerwidth + layerwidth)); // End of Latitude
                            if (Isdebug) debuglist.Add(latstart + tmplatindex * layerwidth + layerwidth);
                        }
                        else
                        {
                            outputwriter.Write((ushort)latend);
                            if (Isdebug) debuglist.Add(latend);
                        }

                        if ((lonstart + tmplonindex * lonlayerwidth + lonlayerwidth) < lonend)
                        {
                            outputwriter.Write((ushort)(lonstart + tmplonindex * lonlayerwidth + lonlayerwidth));    // End of Longitude
                            if (Isdebug) debuglist.Add(lonstart + tmplonindex * lonlayerwidth + lonlayerwidth);
                        }
                        else
                        {
                            outputwriter.Write((ushort)lonend);
                            if (Isdebug) debuglist.Add(lonend);
                        }
                        ...
                        for (j = 0; j < smalllayerblockcount * lonsmalllayerblockcount; j++)
                        {
                            ...
                            outputwriter.Write((ushort)(latstart + tmplatindex * layerwidth + tmplatsmallindex * smalllayerwidth));  // Start Latitude
                            if (Isdebug) debuglist.Add(latstart + tmplatindex * layerwidth + tmplatsmallindex * smalllayerwidth);
                            outputwriter.Write((ushort)(lonstart + tmplonindex * lonlayerwidth + tmplonsmallindex * lonsmalllayerwidth));  // Start of Lon
                            ...
                            {
                                outputwriter.Write((ushort)(latstart + tmplatindex * layerwidth + layerwidth));                // End of Lat
                                if (Isdebug) debuglist.Add(latstart + tmplatindex * layerwidth + layerwidth);
                            }
                            else
                            {
                                outputwriter.Write((ushort)(latstart + tmplatindex * layerwidth + tmplatsmallindex * smalllayerwidth + smalllayerwidth));                // End of Lat
                            ...
                            {
                                outputwriter.Write((ushort)(lonstart + tmplonindex * lonlayerwidth + lonlayerwidth));               // End of Lon
                                if (Isdebug) debuglist.Add(lonstart + tmplonindex * lonlayerwidth + lonlayerwidth);
                            }
                            else
                            {
                                outputwriter.Write((ushort)(lonstart + tmplonindex * lonlayerwidth + tmplonsmallindex * lonsmalllayerwidth + lonsmalllayerwidth));               // End of Lon
            ...
            /*    The version before 2014/03/19/1630
